...
One Last Note: What's shown in this submission is intentionally a stripped-down version of VECTOR, built to fit the five-day window of a Google coursework capstone rather than to demonstrate everything the underlying idea can do. The constraint here was time, not ambition, and the architecture was built to extend well past this prototype. The fuller capability goes considerably further: automated vulnerability testing and remediation running inside a sandboxed environment, exercised against a complex, realistic stack rather than a single asset in isolation, spanning the code layer, the operating system layer, and the application layer together, rather than treating each as a separate problem with a separate tool. That's the harder, more valuable version of this idea, and the five-factor prioritization, human-in-the-loop governance, and closed-loop verification principles built into this prototype were designed from the start to carry over into it. If you're evaluating how AI-orchestrated remediation testing could fit into your organization's security posture, or you're simply curious where this goes next, I'd welcome the conversation. Feel free to reach out and DM me directly.
---
